...
function createTablaUsuarios()
{
    try {
        ...
        else
        {
            ...
            $sql = 'CREATE TABLE `usuarios` (`id` INT NOT NULL AUTO_INCREMENT , `username` VARCHAR(50) NOT NULL , `nombre` VARCHAR(50) NOT NULL , `apellidos` VARCHAR(100) NOT NULL , `contrasena` VARCHAR(100) NOT NULL , `rol` INT(1) DEFAULT 0, PRIMARY KEY (`id`)) ';
            if ($conexion->query($sql))
            {
...
function addRolColumnIfNotExists(){
    $conexion = conectaTareas();
    ...
    if ($result->num_rows == 0) {
            ...
            $sql = 'ALTER TABLE usuarios ADD COLUMN rol INT(1) NOT NULL DEFAULT 0';
            if ($conexion->query($sql) === TRUE) {
                return [true, "Columna 'rol' agregada correctamente."];
            } else {
                return [false, "Error al agregar la columna 'rol': "];
            }
        }
        return [true, "La columna 'rol' ya existe."];  
}
